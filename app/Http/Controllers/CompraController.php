    ...
    public function store(Request $request)
{
    ...
    try {
        ...
        foreach ($request->producto as $index => $productoId) {
            DetalleCompra::create([
                ...
                'costo_compra' => $request->precio_compra[$index],
            ...
            $producto->existencia += $request->cantidad[$index];
            $producto->save();
        }
   ...
   {
    // Obtener la compra junto con los detalles
    //$compra = Compra::with('detalleCompras.producto')->findOrFail($id);

    //return view('compras.detalles', compact('compra'));
    // Obtén la compra junto con los detalles
    $compra = Compra::with('detalleCompras.producto')->findOrFail($id);

      if ($compra)
      {
        return view('compra.compra_detalles', compact('compra'));
